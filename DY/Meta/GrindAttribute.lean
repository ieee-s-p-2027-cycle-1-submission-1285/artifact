...
Otherwise, the default extension name `ext` is unaccessible (macro hygiene).
-/
module
prelude
...
macro (name := _root_.Lean.Parser.Command.registerGrindAttr') doc:(docComment)?
  ...
  `($[$doc:docComment]? public initialize $extId : Extension ← registerAttr $(quote id.getId) (ref := $(quote id.getId))
